  ...
  <div class="layout">
    ...
    <header class="header">
      ...
      <i
        ...
        @click="toggleSidebar"
      ></i>
      <i class="fas fa-bell notification-bell">
  <span v-if="unreadMentionsCount" class="badge">
    {{ unreadMentionsCount }}
  </span>
</i>
...
export default {
  ...
  mounted() {
    ...
    .then(res => this.unreadMentionsCount = res.data.count)
    this.fetchQuestions()
    this.checkIfMobile()
 ...
 onMentionKeyup(e, target) {
  ...
  if (this.showMentionBox && this.mentionUsers.length) {
    ...
    if (e.key === 'Enter' && this.showMentionBox) {
  e.preventDefault()
  ...
  this.setMentionPosition(e.target)

  axios.get('/api/qa/mention-users', {
    params: { q: this.mentionQuery }
  }).then(res => {
...
selectMention(user) {
  let text =
    this.mentionTarget === 'question'
      ? this.newQuestion
      : this.mentionTarget.replyText

  text = text.replace(/@([\w\s]*)$/, `@${user.name} `)

  if (this.mentionTarget === 'question') {
    this.newQuestion = text
  } else {
    this.mentionTarget.replyText = text
  }
