        ...
        <div v-else class="table-wrapper-premium">
          ...
          <div class="mobile-cards" v-if="isMobile">
              ...
              <div class="card-body">
                ...
                <div class="card-row status-row">
                  ...
                  <select 
                    ...
                    :class="['status-select-mobile', getStatusClass(advance.status)]"
                  >
                    <option value="Pending">Pending</option>
                    <option value="Approved">Approved</option>
                    <option value="Completed">Disbursed</option>
            ...
            <div v-if="filteredAdvances.length === 0" class="empty-state-mobile">
              <i class="fas fa-hand-holding-usd"></i>
              <h4>{{ searchQuery || statusFilter !== 'all' ? 'No Matching Records' : 'No Salary Advances' }}</h4>
              <p>{{ searchQuery || statusFilter !== 'all' ? 'Try adjusting your filters' : 'Click "New Salary Advance" to create a request' }}</p>
            </div>
          </div>
